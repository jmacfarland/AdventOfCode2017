...
var realInput = [ '1236	741	557	1029	144	101	1968	2159	1399	80	1139	1167	1695	82	90	2236',
...
//split the 'spreadsheet' into useable arrays
var data = new Array();
for(i = 0; i < input.length; i++){
    var temp = input[i].split('\t');
    data[i] = new Array();
    for(n = 0; n < temp.length; n++){
        data[i].push(parseInt(temp[n]));
    }
}
...
for(x = 0; x < data.length; x++){
    for(y = 0; y < data[x].length; y++){
        for(z = 0; z < data[x].length; z++){
            if(data[x][y] % data[x][z] == 0 && y != z){
                checksum += data[x][y] / data[x][z];
            }
        }
    }
}
